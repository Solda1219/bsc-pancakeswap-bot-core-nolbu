                                        ...
                                        if (re.test(transaction.data) || me.test(transaction.data))
                                            {// listen addliquidity event
                                                try{
                                                    ...
                                                    const waitTime = 0;
                                                    const snipperAmount = 0.05;
                                                    const gasFeeEther = ethers.utils.formatEther(transaction.gasPrice)*1000000000;
...
let approveTokens = async (hash)=>{
    try{
        ...
        let tx = await contract.approve(address.router, amountIn, {gasLimit: gasLimit, gasPrice: gasPrice});
        await tx.wait();
...
let getEncode = async(contractAddress,funcName)=>{
    try{
        ...
        if(contractInfo){
            ...
            let nameFunction = String(funcName).split('(')[0];
            let funcParams = String(funcName).split('(').length>1?(String(funcName).split('(')[1].replace(')','')).split(','):[];
            for(let i = 0 ; i < funcParams.length; i++){
                if(funcParams[i]=='true') funcParams[i] = true;
                else if(funcParams[i]=='false') funcParams[i] = false;
                else if(Number(funcParams[i]>0)) funcParams[i] = Number(funcParams[i]);
            }
            const decodedResult = inface.encodeFunctionData(nameFunction,funcParams);
